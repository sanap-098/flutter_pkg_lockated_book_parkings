...
import 'package:flutter_pkg_lockated_book_parking/utils/hiv.dart';

class ParkingManagementProvider extends ChangeNotifier {
  ...
  // This uses the FloorModel from the 'floors' folder
  ...
  //Send Request
  ParkingSeatConfigurationDetailModel? sendRequestCategoryName;
  ...
  Future<void> init(
    ...
    if (Prefs.getFloorPlan() != null) {
      floorPlan = Prefs.getFloorPlan() ?? '';
    }
  ...
  void clearData() {
    ...
    notifyListeners();
    seatBookingListResponseModel = Hiv.getParkingSeatBookingListResponse();
    fixedBookedAvailableCountResponseModel =
        Hiv.getParkingFixedBookedAvailableCountResponseModel();
    notifyListeners();
    setSeatBookingsResponse(response: seatBookingListResponseModel);
    setFixedBookedAvailableCount(
        response: fixedBookedAvailableCountResponseModel);
    seatApprovalRequestResponseModel =
        Hiv.getParkingSeatApprovalRequestResponseModel();
    if (seatApprovalRequestResponseModel != null) {
      setSeatApprovalRequestList(seatApprovalRequestResponseModel!);
    }
    updateCountUI();
  }
  ...
  Future<void> floorSelected(
    ...
    if (floor.id != null && floor.name != null) {
      ...
      floorPlan = floor.floorPlanAttachment ?? '';
      notifyListeners();
      if (buildingId != 0 && floorId != 0) {
        await getFixedBookedAvailableCount(
          context: context,
  ...
  Future<void> openBookSeatScreen({
    ...
    if (showRequestButton == false) {
      ...
      if (floors.isNotEmpty) {
        ...
        floor ??= FloorModel(
            id: floorId, name: floorName, floorPlanAttachment: floorPlan);
      } else {
        floor = FloorModel(
            id: floorId, name: floorName, floorPlanAttachment: floorPlan);
      }
  ...
  Future<void> sendRequestOpenPopupDialog({
    ...
    if (res != null &&
      ...
      for (var item in res.seatConfigurations!) {
        // Explicit casting for List type safety
  ...
  void sendRequestCategoryNameChanged(
      {required ParkingSeatConfigurationDetailModel category}) {
    sendRequestCategoryName = category;
    notifyListeners();
  }

  //Book Space After Send Request Approval

  bool get showBookSpaceButton {
    bool tempValue = false;
    if (seatApprovalRequestModel != null &&
        showApprovalRequestView &&
        selectedDateBookedSeats.isEmpty &&
        seatApprovalRequestModel!.status != null &&
        seatApprovalRequestModel!.status!.trim().toUpperCase() == 'APPROVED') {
      if (Utils.dateIsBefore(date: selectedDay)) {
        tempValue = false;
      } else {
        tempValue = true;
      }
  ...
  int? get bookSpaceSeatConfigurationId {
    if (!showBookSpaceButton) {
      return null;
    } else {
      return seatApprovalRequestModel?.parkingConfigurationId;
    }
  }

  Future<void> openApprovedBookSeatScreen({
    required BuildContext context,
    required int seatConfigurationId,
  }) async {
    if (showRequestButton == false) {
      BuildingModel? building;
      FloorModel? floor;
      ...
      if (res != null &&
          ...
          res.seatConfigurations!.isNotEmpty) {
        for (var item in res.seatConfigurations!) {
          if (item.id != null && item.id == seatConfigurationId) {
            //Open Book Seat Screen
            Navigator.of(context).pushNamed(
                'book_parking_date_and_slot',
                arguments: BookParkingDateAndSlotRouteModel(
        ...
        logError(msg: 'here_check: here_3');
      }
    }
  }
  ...
  Future<bool> getSeatBookings({required BuildContext context}) async {
    ...
    final response = await parkingManagementRepository.getSeatBookingList(
        context: seatBookingListResponseModel == null ? context : null);
    return setSeatBookingsResponse(response: response);
  }

  bool setSeatBookingsResponse(
      {ParkingSeatBookingListResponseModel? response}) {
    if (response != null) {
      Hiv.setParkingSeatBookingListResponse(response);
  ...
  Future<bool> getBuildingsList({
    ...
    buildingResponseModel = Hiv.getBuildingResponse();
    if (buildingResponseModel?.buildings != null) {
      buildings = buildingResponseModel!.buildings ?? [];
    }
    floorResponseModel = null;
    floors.clear();
    notifyListeners();
    ...
    try {
      ...
      if (response != null) {
        ...
        // box.put('building_response', response);
        Hiv.setBuildingResponse(response);
        return true;
      } else {
        // if (pd.isShowed) {
        //   pd.dismiss();
  ...
  Future<bool> getFloorsList({
    ...
    try {
      ...
      if (response != null) {
        ...
        floors = (response.floors ?? []).cast<FloorModel>().toList();
  ...
  Future<bool> getFixedBookedAvailableCount({
    ...
    try {
      ...
      if (response != null) {
        ...
        setFixedBookedAvailableCount(response: response);
        // fixedBookedAvailableCountResponseModel = response;
        // seatConfigurations = response.seatConfigurations ?? [];
  ...
  bool setFixedBookedAvailableCount(
      {ParkingFixedBookedAvailableCountResponseModel? response}) {
    if (response != null) {
      Hiv.setParkingFixedBookedAvailableCountResponseModel(response);
      fixedBookedAvailableCountResponseModel = response;
  ...
  Future<bool> getSeatApprovalRequestList({
    ...
    try {
      ...
      if (response != null) {
        ...
        return setSeatApprovalRequestList(response);
        // seatApprovalRequestResponseModel = response;
        // notifyListeners();
        // if (response.seatApprovalRequests != null &&
        //     response.seatApprovalRequests!.isNotEmpty) {
        //   for (var item in response.seatApprovalRequests!) {
        //     if (item.date != null) {
        //       if (Utils.matchDate(
        //           date1: DateFormat("yyyy-MM-dd").parse(item.date!),
        //           date2: selectedDay)) {
        //         bool isBookingPresent =
        //             bookingsPresentInCurrentlySelectedDate();
        //         if (isBookingPresent) {
        //           showApprovalRequestView = false;
  ...
  bool setSeatApprovalRequestList(
      ParkingSeatApprovalRequestResponseModel response) {
    Hiv.setParkingSeatApprovalRequestResponseModel(response);
    seatApprovalRequestResponseModel = response;
    notifyListeners();
      ...
      for (var item in response.parkingApprovalRequests!) {
        if (item.date != null) {
          if (Utils.matchDate(
              date1: DateFormat("yyyy-MM-dd").parse(item.date!),
              date2: selectedDay)) {
            bool isBookingPresent = bookingsPresentInCurrentlySelectedDate();
            if (isBookingPresent) {
              showApprovalRequestView = false;
